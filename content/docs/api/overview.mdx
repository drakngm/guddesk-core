...
- **Single workspace key** 鈥?The `workspaceId` is fixed; no need to pass it in requests.
- **All workspaces key** 鈥?You must include `workspaceId` as a query parameter or in the request body for workspace-scoped endpoints.
...
> **Note:** Webhook registration requires a **Full Access** API key. If your agent only needs to read/write articles and reply to conversations, a **Read & Write** key is sufficient 鈥?but you'll need to register the webhook manually in the dashboard.
...
When self-hosting, use `POST /api/mcp` on your own origin. Authenticate with the same API key as the REST API:
...
The stdio entry point lives in this repository at [`bin/guddesk-mcp.ts`](/bin/guddesk-mcp.ts); a `guddesk-mcp` package is not published to npm yet. Run it from a checkout:

```
GUDDESK_API_KEY=gd_abc123... npx tsx bin/guddesk-mcp.ts
```

Then point Claude Desktop (`claude_desktop_config.json`) at that file:
...
MCP speaks JSON-RPC 2.0. Over HTTP, call `list_conversations` with your API key:

```bash
curl https://guddesk.com/api/mcp \
  -H "Authorization: Bearer gd_abc123..." \
  -H "Content-Type: application/json" \
  -d '{
    "jsonrpc": "2.0",
...
Send `"method": "tools/list"` (no params) to discover every tool.

### Available Tools

The MCP server exposes these 26 tools:
...
| `list_customer_events` | List timeline events for a customer |
| `list_companies` | List companies in the workspace |
| `get_company` | Get a company's details including its customers |
| `create_company` | Create a new company in the workspace |
| `update_company` | Update a company's name, domain, website, logo, industry, or metadata |
| `list_segments` | List customer segments in the workspace |
| `get_segment` | Get a customer segment's details and conditions |
| `create_segment` | Create a customer segment with filter conditions (conditions are AND-ed) |
...
- [Webhooks](/docs/api/webhooks) 鈥?Receive real-time event notifications
- [gud-agent](https://github.com/gudlab/gud-agent) 鈥?Open-source AI support agent
