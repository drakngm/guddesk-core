...
We chose AGPL-3.0 specifically because it is copyleft for network use. If you modify GudDesk and offer it as a hosted service, you share those changes. That keeps a competing hosted Intercom-clone from taking the core closed.

## The Business Model

Open source doesn't mean we work for free. Our model is simple:

1. **Self-host (AGPL-3.0)** — Run the core on your own infrastructure. No per-seat fee.
2. **Cloud Free** — 2 members, 500 conversations/month, forever.
3. **Cloud Pro** — $29/mo flat. Unlimited seats, built-in AI, branding removal.
4. **Cloud Business** — ~$99/mo flat. Same no-per-seat rule for larger hosted teams.
